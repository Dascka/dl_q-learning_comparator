from agents.OpenAI.CartPole.dqn import train as DQN
from agents.OpenAI.CartPole.ddqn import train as DDQN
from agents.OpenAI.CartPole.dueling import train as Dueling_DQN
from agents.OpenAI.CartPole.dueling_ddqn import train as Dueling_DDQN
from agents.OpenAI.CartPole.random import train as random
                                ...
                                for warmup in warmups:
                                    ...
                                    if i >= start and i < stop:
                                        for l in range(start_repeat, repeat):
                                            config.seed = l
                                            path = "./Results/CartPole/" + algname + "/graph/"+"lr"+str(lr)+"/disc"+str(discount)+"/eps"+str(epsilon)+"/eps_dec" + str(eps_decay) + "/b_s" + str(batch_size) + "/" +str(i) + "/" + str(l) + ".pdf"
                                            path_csv = "./Results/CartPole/" + algname + "/graph/"+"lr"+str(lr)+"/disc"+str(discount)+"/eps"+str(epsilon)+"/eps_dec" + str(eps_decay) + "/b_s" + str(batch_size) + "/" +str(i) + "/csv/" + str(l)  + ".csv"
...
def graphs(start_path = "Results/CartPole/", algo = 0, start_repeat = 0, repeat = 100):
    '''
    Function for benchmarking for OpenAI CartPole that creates the graphs.
                        ...
                        for batch_size in batch_sizes:
                            for max_step in max_steps:
                                for warmup in warmups:
                                    ...
                                    if algo == -1: 
                                        title = "Résultats de l'algorithme aléatoire pour Cartpole"
...
def multi_graphs(start_path = "Results/CartPole/", DQN_n = 0, DDQN_n = 0, Dueling_n = 0, Dueling_DDQN_n = 0, repeat = 100, start_repeat = 0):
    '''
    Function for benchmarking for OpenAI CartPole that creates the graphs.
    ...
    ep_avgx_mean_plot_multiple([DQN_returns, DDQN_returns, Dueling_returns, Dueling_DDQN_returns, random_returns], title = "Comparaison d'algorithmes sur CartPole", algnames = ["DQN", "DDQN", "Dueling DQN", "Dueling DDQN", "Aléatoire"], path =  path, verbal = True, ylim=ylim, with_variance = True, n_run = repeat, stop_cond = stop_cond, win_cond = win_cond)

def multi_graphs_reading_single(start_path, index = 0, lrs = [0.001, 0.00025], discounts = [0.999, 0.95], epsilons = [1], eps_decays = [0.9, 0.9999], eps_mins = [0.01], batch_sizes = [32], max_steps = [1000], warmups = [1000], repeat = 100, start_repeat = 0):
    '''
    Function for benchmarking for OpenAI CartPole that reads the data for one set of hyperparameters.
    :param path: Start of the path where we can find the csv and will save eps files. Should be the path before the algorithms folders supposed that the algorithms' folders are named "DQN", "DDQN", "Dueling_DQN", "Dueling_DDQN" and "Aléatoire" followed by a folder named graph.
    ...
    return returns, steps #return the returns and steps for every repeat of the set of hyperparameters specified

def multi_graphs_same_algo(start_path = "Results/CartPole/", index = [0], algo = 0, repeat = 100, start_repeat = 0):
    '''
    Function for benchmarking for OpenAI CartPole that creates the graphs.
    :param path: Start of the path where we can find the csv and will save eps files. Should be the path before the algorithms folders supposed that the algorithms' folders are named "DQN", "DDQN", "Dueling_DQN", "Dueling_DDQN" and "Aléatoire" followed by a folder named graph.
    ...
    ep_avgx_mean_plot_multiple(returns, title = "Comparaison de " + algname + " sur CartPole", algnames = algnames, path =  path, verbal = True, ylim=ylim, with_variance = True, n_run = repeat, stop_cond = stop_cond, win_cond = win_cond)

def multi_graphs_reading_multiple(start_path, index = [0], lrs = [0.001, 0.00025], discounts = [0.999, 0.95], epsilons = [1], eps_decays = [0.9, 0.9999], eps_mins = [0.01], batch_sizes = [32], max_steps = [1000], warmups = [1000], repeat = 100, start_repeat = 0):
    '''
    Function for benchmarking for OpenAI CartPole that reads the data for multiple set of hyperparameters.
    :param path: Start of the path where we can find the csv and will save eps files. Should be the path before the algorithms folders supposed that the algorithms' folders are named "DQN", "DDQN", "Dueling_DQN", "Dueling_DDQN" and "Aléatoire" followed by a folder named graph.
    :param index: Index of the selected set of hyperparameter.
    :param lrs: List of learning rates.
    :param discounts: List of discounts.
